 ...
 * the License.
 */
package com.nitrous.gwt.earth.client.api.ajaxloader;
...
public class AjaxLoader {
	...
	public static class AjaxLoaderOptions extends JavaScriptObject {
		...
		protected AjaxLoaderOptions() {
			// A protected constructor is required for JavaScriptObject
			// overlays.
		}
		...
		private native void setCallback(Runnable onLoad) /*-{
			this.callback = function() {
				@com.nitrous.gwt.earth.client.api.ajaxloader.ExceptionHelper::runProtected(Ljava/lang/Runnable;)(onLoad);
			}
		}-*/;
	}
	 ...
	 *            API key value.
	 * @param hostname
	 *            Custom hostname (e.g. "www.google.com").
	 */
	public static void init(String apiKey, String hostname) {
		...
		if (apiKey == null) {
			// No key was specified. Use the location in the page to lookup a
			// key from
			// the Key repository
	 ...
	 * @param onLoad
	 *            A callback that will be invoked when the API is finished
	...
	private static native boolean nativeCreateCallback() /*-{
		...
		$wnd.__gwt_AjaxLoader_onLoad = function() {
			@com.nitrous.gwt.earth.client.api.ajaxloader.AjaxLoader::onLoadCallback()();
		}
	...
	private AjaxLoader() {
		// This class only contains static methods and should not be
		// instantiated.
	}
}
